...
class SignupFragment : Fragment() {
    ...
    private fun initAll() {
        ...
        with(binding)
        {
            ...
            loginTxt.setOnClickListener { Navigation.findNavController(loginTxt).navigate(R.id.action_signupFragment_to_signinFragment) }
        }
    }
        ...
        auth.createUserWithEmailAndPassword(email, password)
            .addOnCompleteListener(requireActivity()) { task ->
                if (task.isSuccessful) {
                    ...
                    database.reference.child("Users").child(id!!).setValue(userModel).addOnCompleteListener {
                        if(task.isSuccessful) {
                            showSnackbar("Signup Successfull")
                            hideProgressBar()
